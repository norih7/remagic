  ...
  return (
    <article>
      ...
      <section className="mb-12">
        ...
        <ol>
          <li>
            今までより長期戦のダンジョンになるのでアイテムや料理でHP/TPを回復できるように準備はしておきましょう。
          </li>
          <li>
            ダンジョンは基本的に道なりに進んでいけますが、浅い水面を歩いたり木に隠れた部分を通るなど少しわかりにくいです。見た目に惑わされないように！また隠れている部分にアイテムなどがあることも（後述）
            <Information type="warning" title="ボイズンチェック">
              水晶霊の河の宝箱には毒状態を100%防止するポイズンチェックが入っています（2つ存在）。ダンジョン内のモンスターは毒状態にする攻撃を多くしてきますのでポイズンチェックを入手したら装備することをおすすめします。
            </Information>
          </li>
          <li>
            奥地ではボス「ウンディーネ」との戦闘になり、勝利後にはウンディーネと契約を結びます。勝利後のイベントではフリンジでスプレッドを習得します。
          </li>
          <li>
            次は王都インフェリアを目標となり、途中の「いざないの密林」へと向かうことに。いざないの密林は序盤の難関ダンジョンであるため、モルルへ戻って念入りにアイテムや食材の準備を整えておいてください。
          </li>
        </ol>
        ...
        <div className="mb-4">
          ...
          <p>
            ダンジョン入り口マップにある木の茂みにはレンズがあります。これは完全に操作キャラクタが隠れてしまう状態。印をつけた箇所で○ボタン連打しましょう。
          </p>
        </div>
        <div className="mb-4">
          <ResponsiveImage src="/stories/guide1-crystal-river-1.jpg" />
          <p>
            ウンディーネが待ち構えているフロアの前には見落としやすいアイテムが存在しています。特に「チンクエデア」は木の茂みにほぼ隠れています。チンクエデアは水属性の短剣で、火属性特攻武器となるためぜひ入手しておきましょう。
          </p>
        </div>
